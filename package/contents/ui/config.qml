...
// 根必须为 Item，以便 Plasma 注入 configDialog / wallpaperConfiguration 初始属性（ColumnLayout 无这些属性会导致报错）
Item {
    id: root

    // 供框架注入，避免 "Setting initial properties failed"
    ...
    function writeWallpaperConfigImage(path) {
        if (!path) return
        cfg_Image = path
        var conf = (wallpaperConfiguration && wallpaperConfiguration.configuration) ? wallpaperConfiguration.configuration : ((typeof wallpaper !== "undefined" && wallpaper && wallpaper.configuration) ? wallpaper.configuration : null)
        if (conf) {
            conf.Image = path
    ...
    ColumnLayout {
        ...
        WallpaperBackend {
            ...
            Component.onDestruction: backend.disconnectFromKabegame()
        ...
        // 不在 QML 中再调 connectToKabegame：C++ 构造里已调用，重复调用易在第二次打开配置时与旧实例销毁产生竞态

        Kirigami.FormLayout {
            Layout.fillWidth: true
            // 显式设置避免 wideMode 绑定循环导致 plasmashell 卡死
            wideMode: true
